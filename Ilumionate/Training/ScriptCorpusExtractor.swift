...
enum ScriptCorpusExtractionFlag: String, Codable, Hashable, Sendable {
...
struct ExtractedScriptBlock: Identifiable, Codable, Hashable, Sendable {
    let id: String
    ...
    let wordCount: Int
    let qualityFlags: Set<ScriptCorpusExtractionFlag>
}

struct ScriptCorpusExtractionIssue: Identifiable, Codable, Hashable, Sendable {
    let id: String
    let sourcePath: String
    let message: String
}

struct ScriptCorpusExtractionReport: Codable, Sendable {
    let blocks: [ExtractedScriptBlock]
    let issues: [ScriptCorpusExtractionIssue]
}

enum ScriptCorpusExtractor {
    static func extract(
        from directory: URL,
    ...
    static func extract(from url: URL, sourcePath: String? = nil) throws -> ScriptCorpusExtractionReport {
    ...
    static func extract(
        ...
        for range in ranges {
            ...
            let wordCount = ScriptPhaseTextAnalyzer.tokens(in: cleanup.spokenText).count
