...
extension AIAnalysisManager {
    ...
    func buildTranscriptionPrompt(
        ...
        var prompt = "Analyze this audio content for synchronized light-pattern generation:\n\n"
        ...
        prompt += "- Sleep Hypnosis: drowsiness cues, sleep imagery, wind-down pacing, often no wake-up ending\n"
        prompt += "- Frequency Audio: binaural, isochronic, solfeggio, or explicit Hz framing\n"
        ...
        prompt += "- Ericksonian metaphor or reframing sections may shift into stories, utilization language, yes-set phrasing like 'that's right', indirect reframes, or invitations to respond in the subject's own way\n"
        ...
        prompt += "- Hypnosis induction: begin around 1.5–2 Hz, then slow toward 0.7–1.2 Hz as the spoken pacing slows\n"
        prompt += "- Erotic Hypnosis: 0.5–1.4 Hz with warm colours and gradual transitions\n"
        prompt += "- Sleep Hypnosis: 0.5–1 Hz, very low intensity, warm colours, typically no closing ramp\n"
        prompt += "- Frequency Audio: use its pacing as context, but never make the visible pattern exceed 3 Hz\n"
        prompt += "- Treat frequency, intensity, and colour strictly as media-control parameters; make no medical, neurological, sleep, stress, mood, cognition, performance, or health claims\n"
        prompt += "- ASMR: subtle 0.8–1.6 Hz pattern with low visual intensity\n"
        prompt += "- Meditation: steady 0.8–1.6 Hz pattern with gradual transitions\n"
        prompt += "- Affirmations: steady 1.2–2 Hz pattern matched to repeated phrases\n"
