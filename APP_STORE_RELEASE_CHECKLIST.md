...
- [x] An iOS 18.5 Simulator **Release** build compiled successfully on 2026-09-01 after the final data-deletion fix.
...
- [x] A signed iOS App Store archive was produced and exported successfully on 2026-09-01; App Store Connect accepted final candidate build `1.0 (10029)` and reports it `VALID`.
- [ ] Produce and validate a signed macOS App Store archive if macOS is part of launch scope.
- [x] Ran Greenlight against the exact build `10029` IPA and triaged every finding. Its “Amplitude” critical is a DSP-word false positive (no Amplitude SDK/domain and `NSPrivacyTracking=false`); its StoreKit warning is `requestReview`, and its launch-storyboard warning ignores the archived app's modern `UILaunchScreen` dictionary.
...
- [x] Build number is `10029` for both the app and share extension; this exact build is uploaded, valid, and attached to iOS version `1.0` in App Store Connect.
- [x] The older TestFlight builds through `1.0 (10028)` are superseded by final candidate build `1.0 (10029)`.
...
- [x] Feature development is frozen. The app-source release candidate is commit `fcd22ee` (shipping behavior through `a3bab52`; `fcd22ee` records build `10029`). Subsequent changes are limited to release records unless a release blocker is found.
...
- [x] Ran the retired-claim scan against the exact build `10029` archive on 2026-09-01. Exact removed claims were absent; internal `brainwaveentrainment` / `EntrainmentBackground` symbol names and the adult/hypnosis classification resources are intentional and are not user-facing claims.
...
- [x] Published the final `PRIVACY_POLICY.md` at public GitHub commit `7233ec4969e576a99b66e7d78e50aaca0866aacc`; its SHA-256 matches the local release policy (`eb3e2d67670c13c9592f96d4f6f95cb80f259b6c173f418021ea82d0be645fc3`).
...
- [x] Reconciled `PRIVACY_POLICY.md` with the final release behavior and published byte-identical text online.
- [x] The policy describes local audio/transcripts/documents/reading state/settings, opt-in camera monitoring, the model download, browser/network access, retention, Clear All Data behavior, and support accurately.
- [x] The policy and App Review notes name TelemetryDeck, Hugging Face/WhisperKit model hosting, and Apple's public catalog lookup and describe what is sent to each.
- [x] Confirmed TelemetryDeck behavior and inspected the exact build `10029` SDK manifest:
  - [x] Product Interaction: collected for Analytics, not linked to the user, not used for tracking.
  - [x] Device ID/app-install identifier: collected for Analytics, not linked to the user, not used for tracking.
- [x] Regression tests verify the analytics SDK does not configure and events do not emit before explicit opt-in.
- [x] Regression tests verify disabling analytics suppresses future events; Clear All Data revokes consent, removes TelemetryDeck state, and suppresses later events.
...
- [x] Code-path audit confirms the camera is requested only when the user enables Reader attention monitoring, with an in-context explanation.
- [x] Camera observations are transient/on-device and capture stops when the app backgrounds, monitoring is disabled, or the Reader disappears.
- [x] Removed the unused Speech Recognition usage description from iOS and macOS plists; the exact build `10029` has no Speech purpose string.
- [x] Audited the exact build `10029` for protected-resource purpose strings: camera is the sole key and its description matches the opt-in Reader feature.
...
- [x] Regression-tested Clear All Data across local audio/library data, documents/support/cache directories, browser data/cookies/cache, preferences, analytics consent/SDK state, and the separately stored downloaded Whisper model directory.
...
- [x] Export compliance is documented as exempt: the app uses only Apple/platform transport and dependency cryptography and implements no proprietary or non-exempt encryption. Build `10029` reports `usesNonExemptEncryption=false` in App Store Connect.
...
- [x] Set `MARKETING_VERSION = 1.0` and `CURRENT_PROJECT_VERSION = 10029` for the app and share extension.
- [x] Verified build `10029` display name, app/extension bundle IDs, signing team, app group entitlement, iOS 18.0 deployment target, and distribution signing in the exported IPA.
- [x] Created and exported the signed iOS **Release** archive for build `1.0 (10029)` with Xcode 26.6 / iOS 26.5 SDK on 2026-09-01. macOS is not in this release scope.
- [ ] Run Xcode Organizer/App Store validation and resolve every error and actionable warning.
- [x] Inspected build `10029`'s exported `.app` for retired claims, the removed known-audio catalog/browser resources and titles, version alignment, privacy manifests, and signing integrity.
- [x] App and share-extension dSYMs are present in the build `10029` archive and match their binaries' UUIDs.
- [x] Recorded the exact uploaded IPA: 30,033,961 bytes, SHA-256 `74f34903b2717abc4f0f306c149432b1c0fc9b58d27e959a128b3a6ec7d27c68`.
- [x] Verified the exact arm64 binary contains no `__LLVM`/bitcode segment; the release configuration contains no active bitcode setting.
- [x] The archive/IPA size was recorded, and the optional approximately 140 MB WhisperKit model download is disclosed before use in App Review notes and the privacy policy.
- [x] Created standard iOS Distribution certificate `A28PFDVL7H` and scoped App Store profiles `ZKWPK5A85H` (app) / `5AKZW6K592` (share extension), all expiring 2027-09-01, after reboot invalidated Xcode's cloud-signing session.

### Automated tests and static checks

- [x] Complete serial iOS 18.5 suite passed on 2026-09-01 after the final model-deletion fix: **1700 tests / 261 suites / 0 failures** in 36.777 seconds.
...
- [x] Ran Greenlight preflight with the exact build `10029` IPA. Its Amplitude/ATT result is a DSP-word false positive (no Amplitude dependency/framework/domain and tracking is false), StoreKit is used only for review requests, the HTTP URL is a test fixture, and the launch warning ignores `UILaunchScreen`; no actionable binary finding remained.
- [ ] Run dependency/vulnerability and license checks for Swift packages and model assets.
- [x] `git diff --check` passes; the release commits contain no merge markers/generated signing material, and a credential scan found no temporary API key, issuer, or private-key material in the repository.
...
- [x] Uploaded build `1.0 (10029)`, whose marketing version exactly matches the App Store version.
- [x] Build `10029` supersedes every earlier `1.0` candidate through `10028`.
- [x] App Store Connect finished processing build `10029` with state `VALID`; the internal Alpha group has automatic access to every valid build.
...
- [x] Attached build `10029` to the iOS 1.0 App Store version.
- [x] Published the complete English (U.S.) TestFlight “What to Test” notes on build `10029`; App Store Connect text matches local SHA-256 `e22ff0169005d8f4f859e3040853c51dd7a2af854e710d0f1c676a01027671da`.
- [ ] Upload, test, and attach a matching macOS build only if Mac is in launch scope.
- [x] Strict TestFlight validation for build `10029` reports zero errors, warnings, or blockers. Strict App Store validation after attachment reports exactly one blocker: `availability.missing`; manual release and API-unverifiable privacy publish state are informational.
